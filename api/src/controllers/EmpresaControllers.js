...
class EmpresaControllers {
  ...
  static async quantidadeEmpresasAtivas(req, res) {
    try {
      ...
      const competencia =
        await EmpresaControllers.obterCompetencia(
      ...
      return res.status(200).json({
        ...
        quantidade: {
          ...
          estabelecimentos: Number(
            dados.total_estabelecimentos_ativos,
          ),
      ...
      console.error(
        'Erro ao contar empresas ativas:',
        error,
      );
  ...
  static async quantidadeEmpresasPorCnae(req, res) {
    try {
      ...
      console.error(
        'Erro ao agrupar empresas por CNAE:',
        error,
      );
  ...
  static async listarEmpresasPorCnae(req, res) {
    try {
      const cnae = String(req.params.cnae || '')
        .replace(/\D/g, '');
      ...
      if (cnae.length !== 7) {
        return res.status(400).json({
          message:
            'O CNAE deve conter exatamente sete números.',
        });
      }
      ...
      return res.status(200).json({
        filtros: {
          cnae,
          cnae_formatado:
            EmpresaControllers.formatarCnae(cnae),
          uf,
      ...
      console.error(
        'Erro ao listar empresas por CNAE:',
        error,
      );
